...
5) Run the `GRADING_SCRIPT` (in `parse.py`) and serialize the output into `VariantResult` objects
...
8) Repeat step 5
9) Compare the `VariantResult` object generated by the student's submission to that of the instructor's solution and report results to Prairielearn
